...
function qualityRules(tier: QualityTier): string[] {
  const rule13: Record<QualityTier, string> = {
    min:
      '13. **只要用户让你"生成 / 写一份 / 整理出一份导图"，默认就把图写完整**' +
    ...
    max:
      '13. **只要用户让你"生成 / 写一份 / 整理出一份导图"，默认就按最高规格来**（除非他说了具体范围）——' +
  ...
  const selfCheck: Record<QualityTier, string> = {
    ...
    high:
      ...
      '叶子节点都是具体知识点；发现哪个板块还是一两层空壳，就回去把它补透；' +
      '最后对照第 13 条 ⑥ 的 80 分三件事（层次逻辑 / 重点区分 / 叶子粒度）过一遍，过不了就不许总结；',
    max:
      '③ 收尾**最严自检**：① 用 findIncompleteNodes 查叶子是否都有实质内容；' +
      '② 用 getDocStats 核对数量与板块覆盖；③ 逐板块对照第 13 条 ②~⑤（三层深度、叶子两类补充、' +
      '每板块 ≥2 例子、易混成对对比）；④ **通读一遍找「外行味」节点**——只点题不给人信息、' +
      '或换个领域也说得通的泛泛之谈，当场替换重写；'
  }
  const rule14 =
    '14. **工作方式：先计划 → 逐步动手并反馈 → 最后自检**（用户会在界面上看到这条时间线）：' +
    '① 大任务（新建整张图、批量补内容、重构结构）**第一步就调用 updatePlan**，写出 2~6 步计划；' +
...
export function buildChatSystemPrompt(input: {
  ...
  totalNodes: number
  sheetCount: number
  /** 生成质量档位（min / high / max）；不传按默认档（high） */
  tier?: QualityTier
  /**
  ...
  return [
     ...
     */
    /**
     * 第 13 / 14 条：**按质量档位生成**（min 及格省 token / high 标准 80 分 / max 更细 90 分）。
     * 见 `qualityRules`——三档共用底线，只在规模、深度、自检强度上分岔。
     */
    ...qualityRules(normalizeQualityTier(input.tier ?? DEFAULT_QUALITY_TIER)),
    /**
     * 第 15 条：**信息不足以判断时，先问再动手**——这是通用原则，不只限某个场景。
     * 规则 3/4 管的是「节点指向」，这条管其余一切：任务本身的大方向（考试科目 / 级别 /
     * 版本 / 领域）、范围与详略、成败判据……只要现有信息推不出唯一合理解读，就问。
     * 真事：用户说「生成计算机四级复习大纲」，模型擅自挑了网络工程师方向写 100+ 节点——
     * 几十步工具调用全花在错的目标上。猜对的代价是运气，猜错的代价是全部返工。
     */
    '15. **信息不足以判断时，先问再动手**：动手前先自问「用户想要什么，我从现有信息能唯一确定吗？」——' +
      '目标不明、范围不清、存在多种合理解读、关键参数缺失（考试科目 / 级别 / 版本 / 领域 / 详略程度等），' +
      '都算**不足以判断**：用 askUser 问清关键分歧点（给出候选选项），得到答复再动手。' +
      '**靠猜的代价远大于问**。反过来也一样：能唯一定位、能从上下文合理推出的就**直接做**，' +
      '不要拿提问当拖延——判断标准始终是「信息够不够」，不是「问着安不安全」。'
  ].join('\n')
}
