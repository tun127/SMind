...
export function buildChatSystemPrompt(input: {
  ...
  return [
    ...
    '13. **只要用户让你"生成 / 写一份 / 整理出一份导图"，默认就按详细规格来**' +
      ...
      '**保质保量是第一优先**，合格线是「60 分」：行家看第一层就认得出「这是内行划分」；' +
      '没有任何空壳板块；节点内容要有行家才会写的细节——覆盖不全、内容掺假，等于没做：' +
    ...
    /**
     * 第 14 条：工作方式「计划 → 工具 → 每步反馈 → 自检总结」。
     *
     * 关键差别：**计划要走工具（updatePlan），而不是"先说一段话"**。
     * 说一段话没有任何约束力，下一步它就可能跑偏；而清单会出现在界面上、
     * 也留在对话里，模型每轮都能看到自己走到哪一步——这正是长任务最缺的东西。
     * 自检同理：用 findIncompleteNodes 把"漏在哪"变成一份带句柄的清单，
     * 而不是让它凭印象说"都写好了"。
     */
    '14. **工作方式：先计划 → 逐步动手并反馈 → 最后自检**（用户会在界面上看到这条时间线）：' +
      ...
      '叶子节点都是具体知识点；发现哪个板块还是一两层空壳，就回去把它补透，然后才许总结；' +
      '发现问题**当场修正**，不要瞒报；' +
