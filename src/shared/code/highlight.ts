...
const C_KEYWORDS = [
  'auto', 'break', 'case', 'class', 'const', 'continue', 'default', 'do', 'else', 'enum', 'explicit', 'export',
  'friend', 'goto', 'if', 'import', 'inline', 'namespace', 'new', 'operator', 'private', 'protected', 'public',
  'register', 'return', 'sizeof', 'static', 'struct', 'switch', 'template', 'this', 'throw', 'try', 'catch',
  'typedef', 'typename', 'union', 'using', 'virtual', 'volatile', 'while', 'override', 'final'
]
...
const DEFS: Record<string, LanguageDef> = {
  javascript: {
    ...
    keywords: [
      ...
      'typeof', 'var', 'void', 'while', 'with', 'yield', 'as', 'await'
    ],
    literals: ['true', 'false', 'null', 'undefined', 'NaN', 'Infinity'],
    builtins: [
      'Array', 'Boolean', 'Date', 'Error', 'JSON', 'Map', 'Math', 'Number', 'Object', 'Promise', 'RegExp', 'Set',
      'String', 'Symbol', 'console', 'document', 'window', 'module', 'require', 'process', 'exports'
    ]
  },
  typescript: {
    mode: 'code',
    ...
    keywords: [
      ...
      'constructor', 'continue', 'declare', 'default', 'delete', 'do', 'else', 'enum', 'export', 'extends',
      'finally', 'for', 'from', 'function', 'get', 'if', 'implements', 'import', 'in', 'infer', 'instanceof',
      'interface', 'is', 'keyof', 'let', 'namespace', 'never', 'new', 'number', 'object', 'of', 'override',
      'private', 'protected', 'public', 'readonly', 'return', 'satisfies', 'set', 'static', 'string', 'super',
      'switch', 'symbol', 'this', 'throw', 'try', 'type', 'typeof', 'undefined', 'unique', 'unknown', 'var',
      'void', 'while', 'yield'
    ],
    literals: ['true', 'false', 'null', 'undefined', 'NaN', 'Infinity'],
    builtins: [
      'Array', 'Boolean', 'Date', 'Error', 'JSON', 'Map', 'Math', 'Number', 'Object', 'Promise', 'RegExp', 'Set',
      'String', 'Symbol', 'WeakMap', 'console', 'document', 'window', 'process', 'Record', 'Partial', 'Readonly'
    ],
    upperAsType: true
  },
  python: {
    ...
    builtins: [
      'abs', 'all', 'any', 'bool', 'bytes', 'dict', 'dir', 'enumerate', 'filter', 'float', 'format', 'frozenset',
      'getattr', 'hasattr', 'int', 'isinstance', 'issubclass', 'iter', 'len', 'list', 'map', 'max', 'min', 'next',
      'object', 'open', 'print', 'range', 'repr', 'reversed', 'round', 'self', 'set', 'setattr', 'sorted', 'str',
  ...
  java: {
    ...
    builtins: ['String', 'System', 'Integer', 'Long', 'Double', 'Boolean', 'Object', 'List', 'Map', 'Set', 'Math', 'Override'],
    upperAsType: true
  },
  csharp: {
    mode: 'code',
    ...
    keywords: [
      ...
      'explicit', 'extern', 'finally', 'fixed', 'float', 'for', 'foreach', 'get', 'goto', 'if', 'implicit', 'in',
      ...
      'set', 'short', 'sizeof', 'stackalloc', 'static', 'string', 'struct', 'switch', 'this', 'throw', 'try',
      'typeof', 'uint', 'ulong', 'unchecked', 'unsafe', 'ushort', 'using', 'var', 'virtual', 'void', 'volatile',
      'while', 'yield', 'nameof', 'when'
    ],
    literals: ['true', 'false', 'null'],
    builtins: ['Console', 'String', 'Int32', 'Double', 'Boolean', 'Object', 'List', 'Dictionary', 'Task', 'Math', 'Enumerable'],
    upperAsType: true
  },
  c: {
    mode: 'code',
    ...
    builtins: ['printf', 'scanf', 'malloc', 'free', 'sizeof', 'strlen', 'memcpy', 'FILE', 'size_t', 'uint8_t', 'int32_t'],
    upperAsType: true
  },
  cpp: {
    mode: 'code',
    ...
    blockComment: [['/*', '*/']],
    strings: ['"', "'"],
    keywords: [...C_KEYWORDS, 'nullptr', 'constexpr', 'noexcept', 'concept', 'requires', 'co_await', 'co_return'],
    literals: ['true', 'false', 'nullptr', 'NULL'],
    builtins: ['std', 'cout', 'cin', 'endl', 'string', 'vector', 'map', 'set', 'unique_ptr', 'shared_ptr', 'size_t', 'auto'],
    upperAsType: true
  },
  go: {
    mode: 'code',
    ...
    builtins: [
      'append', 'cap', 'close', 'complex', 'copy', 'delete', 'error', 'float32', 'float64', 'int', 'int64', 'len',
      'make', 'new', 'panic', 'print', 'println', 'recover', 'rune', 'string', 'uint', 'uint64', 'byte'
    ],
    upperAsType: true
  },
  rust: {
    ...
    builtins: ['String', 'Vec', 'Box', 'Option', 'Result', 'println', 'print', 'format', 'vec', 'panic', 'assert', 'assert_eq'],
    upperAsType: true,
    sigils: ['#']
  },
  sql: {
    ...
    keywords: [
      ...
      'foreign', 'from', 'full', 'group', 'having', 'if', 'in', 'index', 'inner', 'insert', 'into', 'is', 'join',
      'key', 'left', 'like', 'limit', 'not', 'null', 'offset', 'on', 'or', 'order', 'outer', 'primary',
      'references', 'right', 'select', 'set', 'table', 'then', 'union', 'unique', 'update', 'values', 'view',
      'when', 'where', 'with'
    ],
    literals: ['true', 'false', 'null'],
    builtins: ['avg', 'count', 'coalesce', 'max', 'min', 'now', 'sum', 'int', 'text', 'varchar', 'date', 'timestamp'],
    ignoreCase: true
  },
  json: { mode: 'data', strings: ['"'], literals: ['true', 'false', 'null'] },
  yaml: { mode: 'data', lineComment: ['#'], yamlKeys: true, strings: ['"', "'"], literals: ['true', 'false', 'null', 'yes', 'no', '~'] },
  bash: {
    mode: 'code',
    lineComment: ['#'],
    strings: ['"', "'"],
    keywords: [
      'alias', 'case', 'cd', 'do', 'done', 'echo', 'elif', 'else', 'esac', 'eval', 'exec', 'exit', 'export',
      'fi', 'for', 'function', 'if', 'in', 'local', 'read', 'readonly', 'return', 'set', 'shift', 'source',
      'then', 'unset', 'until', 'while'
    ],
    literals: ['true', 'false'],
    builtins: ['awk', 'cat', 'chmod', 'curl', 'find', 'git', 'grep', 'head', 'ls', 'mkdir', 'npm', 'printf', 'rm', 'sed', 'sort', 'sudo', 'tail', 'touch', 'tr', 'wc'],
    sigils: ['$']
  },
  html: { mode: 'markup' },
  css: { mode: 'css' }
...
function scanCodeLine(line: string, def: LanguageDef, state: ScanState): CodeToken[] {
  ...
  const has = (list: string[] | undefined, word: string): boolean =>
    Boolean(list?.includes(word))
...
export function highlightCode(code: string, language: string | undefined): CodeLine[] {
  ...
  if (!def) return lines.map((line) => ({ tokens: line.length > 0 ? [{ text: line, kind: 'plain' as const }] : [] }))
