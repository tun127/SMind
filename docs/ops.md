...
| 7   | **国内镜像**（改用阿里云 OSS 香港；R2 搁置） | 🟡 **桶与对象已就绪，卡在 `dl.smindapp.cn` 的 CNAME 目标写错**            | 2026-09-20：R2 因**无可用支付方式**搁置 → 改**阿里云 OSS 香港 + 绑 `dl.smindapp.cn`**（香港地域免 ICP 备案、支付宝可付；手册 `docs/release-oss.md`）。已完成：桶 `smind-releases` 已建、上传脚本 `npm run mirror:oss` 已入库、**4 个对象已上传且可匿名读**（直连 OSS 端点实测 `latest.yml` 内容为 `version: 0.9.1`；桶根 403 = 禁止列桶，正常）。**当前唯一故障（2026-09-20 实测）**：Cloudflare 里 `dl` 的 CNAME 目标写成了 `smind-releases.cn-hongkong.thatapicdn.com` —— 而 `thatapicdn.com` **整个域都不存在**（NXDOMAIN，阿里 DNS 与 CF 权威 NS 一致），CNAME 链断在末端 → `dl.smindapp.cn` 解析不出任何地址；**正确值 `smind-releases.oss-cn-hongkong.aliyuncs.com`**（实测解析到 47.79.66.58 / 106.11.35.29）。改对后还差两步：② **HTTPS 证书**（阿里云免费 DV，**有效期 90 天须重申请**；OSS 域名管理里托管）③ **OSS CORS 规则**（`smindapp.cn` → `dl.smindapp.cn` 的 `fetch` 探测是跨域，漏配 CORS 会静默回退 GitHub） | 用户 |
| 8   | 官网下载页                                   | ✅ 镜像优先 + GitHub 回退（已上线；镜像未就绪时自动走回退分支，实测有效） | ⚠️ 回退路径**可用但很慢**：GitHub 直链国内实测 **0.02 MB/s**，108 MB 安装包约 **90 分钟** —— 所以 #7 值得尽快修完（这是"用户能不能装上"的问题，优先于购买入口之外的其它事项）。页面本身无需改动                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | #7   |
