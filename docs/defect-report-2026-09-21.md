...
### 15.2 ⚠️【已推翻】D-07 的"验通"是**假绿**：真实输入法下仍然折行

> **2026-09-21 深夜更正**：用户实测「**不行，没修**」（已重启到安装版 0.9.2.0）。
> 本节原先写的是"验收通过"，**该结论不成立**：我用 `Input.imeSetComposition` 注入的是**合成** composition 事件，
> 它让加宽生效（实测编辑框 240px / 300px、rows=1）；而**真实输入法路径下加宽没有生效**，拼音照样折行。
> **教训（与 §15.3 是同一对，但这次是我自己犯的）**：**合成事件 ≠ 真实输入**；
> "验证通过"必须由**真实输入法**或**等价证据**支撑，做不到就只能记"未验"。
> 下面保留原始记录（**不要再据此判断 D-07 状态**），真实结论见 §18。
>
> **⚠️ 2026-09-21 深夜补充（这条更正本身也要更正）**：用户实测所用的**包**经查是**修复前构建** ——
> - 机器上安装版 `%LOCALAPPDATA%\Programs\SMind\resources\app.asar`（**20:20**）**不含**修复标记 `setComposingWidth`；
> - 本批正式产物 `release/win-unpacked/resources/app.asar`（**22:07**）**含**该标记；
> - D-07 的修复提交时间是 **21:40**，而 `release-rt/` 里那几个 rt 测试包都在修复前打出。
>
> → 因此「用户实测仍折行」**不能直接推翻**上文的 CDP 结论：**两边跑的很可能不是同一个构建**。
> **状态改为「待复验」**：需在 22:07 的正式包（`release/SMind-0.9.2-x64-portable.exe` / 重装 `SMind-0.9.2-x64-setup.exe`）上再验一次，
> 才能判定"合成 PASS、真机 FAIL"是否成立。
> **这条二次更正本身也是教训**：推翻一个结论之前，先确认**被推翻的对象和证据是不是同一个东西**（构建、环境、路径）。
...
### 15.4 ⚠️【已推翻】发版裁决更新 → 见 §15.2 更正：**D-07 在真实输入法下仍未修**（0.9.2 已发布，须在 0.9.3 修）
...
## 18. D-07 真机根因定位（2026-09-21 深夜 · 用真实输入法实测）

> **结论**：D-07 **在真机上确实没修好**，根因**不是**"事件收不到"，而是 **`compositionend` 后宽度立刻回退**。
> 本节数据来自**用户的真实输入法**（不是合成事件），由文书侧用 CDP 观测器采集。

### 18.1 观测设置

带 `--remote-debugging-port` 启动 0.9.2 正式包（`release/win-unpacked`，22:07 构建，**含 §15 的修复**），
用 `Runtime.evaluate` 在 `document` 上挂 capture 监听，记录 `compositionstart/update/end/input` + 每次事件时刻的编辑区几何。
用户**用真实输入法**打字、**不按回车**。

### 18.2 关键读数（80 条事件）

```
组词中（compositionupdate 期间）：
  editorInlineWidth: "220px" → "240px"
  editorFlex:        "0 0 auto"      ← 修复生效 ✓
  editorMaxWidth:    "none"          ✓
最后一次事件：compositionend
结束之后：
  编辑区文本:        "d'd'd'd'd'd'd'd'd'd'd'd'd'd'd'd'd'd'd'd"（39 字符）
  节点框宽:          "76px"          ← 测量没跟上
  编辑区 inline 宽:  "49px"          ← **缩回了**（= 76 - 2×13.5 的节点内宽）
  编辑区 flex:       ""              ← 已复位
  编辑区 maxWidth:   "100%"
  行数:              7               ← 挤成 7 行
```
...
**`compositionend` 一到，`setComposingWidth(0)` 就把加宽撤销；而那串文本此时还没提交进文档、测量也还没跟上** → 编辑框退回节点内宽 → **未提交的拼音被挤成多行**。
用户按 Enter 提交后，测量才包含这段文本、节点框才变大 → 于是现象表现为「**没按 Enter 会扁，按了就正常**」。

**这不是"事件收不到"，恰恰相反：组词期是好的。缺陷住在「组词结束 → 提交」这个中间态里** —— 而这个状态在本产品里是**一等公民**（编辑器是"草稿"，Enter 才提交）。

### 18.4 我的测试为什么会给出假 PASS（记录在案）

我的 `cdp-d07.cjs` 只在 **`imeSetComposition` 之后**量（= 组词**中**）→ 那段确实是一行 ✓。
**我从来没有量过"结束组词但不提交"这一段** —— 用户说的正是这一段。合成事件本身没问题，**是我的取样时点选错了**。

### 18.5 修法方向（给代码侧）

**宽度不该只由"是否在组词"驱动，而应由"编辑区当前内容"驱动**：
1. **最小且对症**：宽度取 **`max(按 node.width 算的宽度, 当前 DOM 内容所需宽度)`**（仍受 `cap` 约束），
   并在 `needed > 测得宽度` 时同时给 `flex: 0 0 auto` / `maxWidth: none`。
   这样"组词中"和"组词结束未提交"两种情况都不会被压窄 —— **不再依赖 `composingWidth` 这个瞬时标志**。
2. **治本（同时让"节点框"跟着变宽）**：把**草稿文本**接进 `use-canvas-layout` 的测量（报告 §D-07 的第 2 档），
   使节点框在组词/草稿期就按内容变宽，而不是等 Enter 提交。
3. 若暂时只做第 1 条：**`compositionend` 不要立刻把宽度归零**，至少要撑到"文档里已经有了这段文本"（测量跟上）之后再复原。

**验收判据（这次必须按这个来）**：
用真实输入法打字后 **不按回车**，量 `.rich-editor__content` 的 `scrollHeight / lineHeight` **必须 = 1**；
再按回车，节点框应变宽到容纳该文本。**只测"组词中"不算通过**（这正是本次假 PASS 的成因）。

---

## 19. D-07 根因彻查（2026-09-22 凌晨 · 以 `v0.9.0` 为"已知正常"基线）

> **触发**：用户判定"0.9.0 的未解耦代码没有问题，要彻查到底"，并要求**撤回仪表侧对 D-07 的全部改动**。
> **已执行**：`ece3c1a` 把今晚仪表侧改过的 9 个文件全部恢复到 `2e8f4ba`（我动手之前的状态）并推送。
> **方法**：不再只看"最近的修复提交"，而是拿 `v0.9.0` 标签当基线，对**整条编辑链路**逐层比字。

### 19.0 一句话结论

**这里混着两个不同的问题，而用户"0.9.0 没问题"的直觉是对的**：

| # | 问题 | 从何时起 | 现场表现 |
|---|---|---|---|
| **P1 · 宽度冻住** | 编辑区宽度只由 `node.width` 决定，而 `node.width` 来自布局测量（吃的是**已提交**的 `editingText`）→ 未提交的组词文本不影响宽度 | **v0.9.0 就有**（**不是回归**） | 拼音在**节点框内**折成多行 |
| **P2 · 溢出框外** | `cb98bd9` 给编辑区加了 `flex: 0 0 auto` + `maxWidth: none` → 编辑区**不再被 flex 收缩**，可以比节点框还宽 | **0.9.2 才引入**（今晚那次"最小可见修"） | 文字**溢出节点边框**、左右都露（`.topic__editor` 是 `justify-content:center`） |

→ **用户截图里的是 P2**；§18 采到的"7 行"是 **P1**。两者被当成同一个 bug 追了三轮，越修越坏。

### 19.1 证据：`v0.9.0` 与 HEAD 的编辑链路逐字对比

| 层 | `v0.9.0` | 现在的 HEAD | 结论 |
|---|---|---|---|
| 编辑器宽度 effect | `dom.style.width = textWidth; maxWidth='100%'`（`v0.9.0:RichTextEditor.tsx` 尾部） | 同（`else` 分支），另加组词分支 | **v0.9.0 完全没有组词处理** → P1 那时就存在 |
| 组词监听 | **无** | `compositionupdate/end` → 改宽度 | `515aac2` 加的（第一代尝试） |
| `flex: 0 0 auto` | **无** | 组词期设置 | `cb98bd9` 加的（第二代）→ **P2 的来源** |
| `.topic__editor` | `width:100%; min-width:0; display:flex; justify-content:center` | **逐字相同** | CSS 不是回归点 |
| `.rich-editor__content` | `width:100%; min-width:0; white-space:pre-wrap; overflow-wrap:anywhere; line-height:1.5` | **逐字相同** | 同上 |
| 布局如何测编辑节点 | `measureTopic({...topic, title: editingText, titleRich: editingRich})`，deps 含 `editingText/editingRich` | 同（只是搬进 `use-canvas-layout.ts`） | 输入相同 |
| 依赖版本 | — | tiptap / prosemirror / react / zustand **无变化** | 排除库升级因素 |

**结论：A3 的"解耦"（`64ed49e` / `0bb4812` / `3fe548d` / `329546a`，以及 `use-canvas-layout` 的抽取）没有改行为** ——
CSS、宽度 effect、测量输入三项逐字一致。**行为变化全部来自 `515aac2` 与 `cb98bd9` 这两次"D-07 修复"本身。**

### 19.2 过程复盘（为什么会走到"越修越坏"）

1. 用户报的是 **P1**（拼音在框内折行）；
2. `515aac2` 用"放开 `max-width`"去修 → 被 flex-shrink 抵消 → **真机无效**（该提交自己的注释也承认）；
3. `cb98bd9` 为绕开 flex-shrink 加了 `flex: 0 0 auto` → **引入 P2（溢出框外）**；同时把宽度改成"组词事件驱动"，
   于是又多出 §18 的"`compositionend` 一缩回就折成 7 行"；
4. 排查阶段把 2/3 的症状（框内折行 / 溢出 / 缩回）都当成同一个 D-07 在追 —— 这是三轮返工的直接原因。

### 19.3 修法：两步**必须成对**，单做任一步都比现状更糟

- **第 1 步（先做，一行级）**：去掉 `cb98bd9` 的 `flex: 0 0 auto` / `maxWidth: none`
  ⇒ 编辑区重新可被 flex 收缩，**文字不再溢出节点框**，回到 `v0.9.0` 的"框内换行"。
  **这一步单独就能消掉用户截图里的丑态。**
- **第 2 步（治本）**：把编辑区 DOM 里的实时草稿喂进布局测量（只喂测量的通道，不进文档、不改格式），
  让**节点框跟着内容长** ⇒ 组词期既不折行也不溢出。
- **为什么必须成对**：只做第 2 步时，布局比 DOM 滞后一帧 / 一个节流窗（`layoutWorkbook` ~100ms），那一瞬编辑区仍可能比框宽 → 依旧溢出；
  只做第 1 步则退回"框内折行"（即用户最初报的 P1）。**"不许逃出框" + "框跟着长" 一起才是闭环。**

> 仪表侧今晚确实实现并自测过第 2 步（CDP 真机：节点框 124 → 360、18 个字符 1 行），
> 但按用户要求已随 `ece3c1a` **全部撤回**，其代码留在 `62468f2`（可 `git show 62468f2` 取回）。
> **未经验收即回滚，不作为交付、不计入"已修"。**

### 19.4 一并被撤回的、与本 bug 无关的一项（必须记得重做）

`ece3c1a` 同时撤销了 `scripts/upload-oss.mjs` 的 **D-20 修复**（清单文件永不按体积跳过 + 收尾校验线上清单版本）。
它与编辑链路无关，是发版链路的 P0（发 0.9.2 时 `latest.yml` 被静默漏传过一次）。
**发 0.9.3 前必须重新落地**，内容见 §16 与 `git show c5ec5ad -- scripts/upload-oss.mjs`。

### 19.5 方法论自省（本轮我犯的两个错）

1. **搜索假阴性**：`Select-String -Path src/**/*.ts` 里的 `**` 在 PowerShell 里**不递归**，
   我据此得出"`setSizeOverride` 已不存在"的结论 —— 实际它只是搬到了 `topic/resize-handle.tsx`。
   → 规矩：**递归搜索一律用 `git grep` / ripgrep，不用 PowerShell 通配符。**
2. **取样点太窄**：我只量了"组词中"，没量"组词结束未提交"，给出过一次假 PASS（§18.4 已记）。
   → 规矩：**几何/时序类缺陷必须量"事件前 / 事件中 / 事件后"三个时点。**

---

## 20. 人工验收清单（D-07 修复后，由**用户本人**执行）

> **触发条件**：代码侧报告"D-07 已修"。QA 先做静态复核（20.4），再由用户按 20.2 执行。
> **为什么必须人工**：本轮三条缺陷全部落在 **CSS × DOM × 时序**上，2852+ 条纯函数断言**结构上盖不住**（§5）。

### 20.1 Bug 清单（本轮只验这三条，别扩范围）

| 编号 | 名称 | 修好后应看到 | 来源 |
|---|---|---|---|
| **P2** | 文字**溢出节点框外**（打字时文字跑到框外，左右都露） | 文字**永远在节点框内**；编辑区宽度 ≤ 节点框宽 + 1px | `cb98bd9` 引入 —— **用户截图里的就是这条** |
| **P1a** | 组词期在**框内折成多行**（拼音没打完就换行） | 组词中保持**一行** | `v0.9.0` 就有（非回归） |
| **P1b** | 组词结束、**未按 Enter** 时折行/缩回 | 保持**一行** | §18 真机实测（旧代码此处 7 行） |

**不需要重复验的**（各有自己的网，别扩大战场）：`.xmind` 往返、AI 侧、许可、自动更新。

### 20.2 验收步骤（约 6 分钟）

**第 0 步 · 先确认装的是"含修复的构建"（上次就栽在这里）**
- 打开应用里能看到版本号的地方（设置 / 关于 / 标题栏），版本**必须是修复批指定的号**（如 `0.9.3`）。
  显示 `0.9.2` = **装的是旧包，验了也白验** —— §15.2 那次就是这么白跑一轮的。
- 把 `release/win-unpacked/SMind.exe`（或安装包）的**文件时间戳**发 QA 核一次：**必须晚于修复提交时间**。

**A 组 · 溢出（P2，最丑的那条）**
1. 双击中心主题进编辑态；
2. 关掉输入法，英文连打 **20 个字符**（如 `dddddddddddddddddddd`），**不按回车**；
3. 看：文字是否**始终在蓝色节点框内**（允许框变宽，**不允许**文字露到框外）。
   - ✅ 通过：文字在框内（框跟着变宽，或文字在框内换行）
   - ❌ 不通过：文字露到框外（左右都能看到超出）→ **P2 没修**

**B 组 · 中文输入法（P1a / P1b —— 你最初报的那条）**
1. 切中文输入法（如微软拼音），双击一个节点进编辑态；
2. 连打 **8 个以上音节**（如 `dawadawdawdaw`），**不按回车、不选词**，停在拼音/候选阶段；
3. 看：拼音是否**始终一行**；
4. 接着**选第一个候选**（组词结束），**仍然不按回车**；
5. 看：候选字是否**仍然一行**（这一步是 §18 抓到的失败点，旧代码在这里挤成 7 行）；
6. 最后按 **Enter** 提交，看：排版与提交前是否一致（不许"跳一下"）。

**C 组 · 回落与回归（防改过头）**
1. 按 Enter 后，节点框是否**回到测量宽度**（不该永久变宽）；
2. 把长标题**全选删除**，看框是否**缩回**（D-14：别为修 D-07 把缩回弄坏）；
3. 再进编辑态打几个字、然后**点画布别处**退出 → 看有没有**残留变宽**；
4. 打开 + 保存一个 `.xmind`（带图片更好）→ 正常（编辑态改动不许污染往返）。

### 20.3 判据速查（可照着量）

| 检查 | 通过线 |
|---|---|
| 文字在框内 | 编辑区 `getBoundingClientRect().width` ≤ 节点框宽 + 1px |
| 是否折行 | 编辑区 `scrollHeight / lineHeight` **=== 1** |
| 提交后是否跳 | 提交前后节点框宽与行数一致 |
| 有无残留 | 退出编辑后节点框宽 === 该标题按测量应得的宽度 |

### 20.4 QA 静态复核（在用户动手之前先做；不合格就不该占用用户的时间）

1. `dom.style.flex` 在仓库里**只能被清空**（`= ''`），**不得**出现 `'0 0 auto'`；
2. `dom.style.maxWidth` 只允许 `'100%'`，**不得**出现 `'none'`；
3. `dom.style.width` 的**写入点只有一处**；
4. 新增的"草稿通道"**不进文档、不改 `editingRich`**（`commitEdit` 只认 `editingText` / `editingRich`）；
5. 组词中 / 组词结束未提交 / 提交后 **三个时点**的真机读数由代码侧提供，**缺一条即视为未验**。

### 20.5 恢复官网下载的触发条件

**全部满足才恢复**（QA 用一条命令回滚 `1ad0e98` 即可）：
1. 20.4 静态复核通过；
2. 用户按 20.2 走完 A/B/C，**无一条不通过**；
3. 代码侧提供三个时点的真机读数；
4. **版本号已递增**（不得用同一版本号重新上传不同字节的产物 —— 否则 updater 不会推送，且同名不同内容）。

---

## 21. 两步修复的静态复核（2026-09-22 · QA，用户验证之前）

> **复核对象**：`26d3743`（第 1 步：编辑区不再逃出节点框）、`4ca0e06`（第 2 步：实时草稿喂进布局测量）
> **复核方式**：只读 diff / `git grep` 定点核对 §20.4 的五条判据 —— **不采信提交信息**
> **结论**：**4/5 通过；第 5 条（真机三时点读数）缺失 ⇒ 按 §20.4 的规则，当前状态仍是"未验"，不得记成已修。**

### 21.1 逐条判据

| # | 判据 | 结论 | 证据 |
|---|---|---|---|
| 1 | `dom.style.flex` 只允许被清空 | ✅ | `RichTextEditor.tsx:211` 只有 `dom.style.flex = ''`（无条件执行）；`'0 0 auto'` **只出现在 :198 的注释里**（说明它为何被删） |
| 2 | `maxWidth` 只允许 `'100%'` | ✅ | 全文件已无 `maxWidth = 'none'` / `'none'` |
| 3 | `dom.style.width` 写入点唯一 | ✅ | `git grep -c` = **1** |
| 4 | 草稿通道不碰文档 / `editingRich` | ✅ | `selection.ts:100` 只写 `editingDraftText`；`types.ts:61` 随 `NO_EDITING` 清空；`use-canvas-layout.ts:95` 用 `editingDraftText \|\| editingText`（`titleRich` 不变，deps 见 :124）；`Canvas.tsx:60/:110` 只做透传 |
| 5 | 三个时点真机读数 | ❌ **缺失** | 代码侧自述"一个都没量" |

### 21.2 ⚠️ 我**撤回**自己此前的两处过强判断（QA 自省，必须记在案）

1. **"canvas 量宽 → 折 2 行 / DOM 探针 → 1 行"这个对照不成立**：复看那两次运行，**样本不同**
   （一次 18 个字符、一次带"中心主题"共 22 个字符，userdata 也不是同一份），因此**不能**据此断言
   "不做探针就必然折 2 行"。
   **仍然成立的部分**：草稿通道确实让**框从 124 长到 360**（两次运行都测到 ✓），而 18 个字符在 313px 里本就放得下
   → **第 2 步很可能已把折行一并解决**，探针只是"最后一个字符"的保险，**不是前提**。
   → 因此**不建议**先补探针再验；**先取三时点读数**，让数字决定要不要探针。
2. **`composition-width.ts` 已成死代码**：应用侧 import 已删，但两个导出仍被
   `scripts/selfcheck/domains/ui.ts:1721-1726` 的 6 条断言测着 —— 这 6 条正是"自检 2872 不变"里的一部分，
   属于**给已删机制留的假账**（§5「全绿 ≠ 没问题」的又一例）。建议随下批清理。

### 21.3 口径纠正

代码侧报"本地待推 **38 条**"，实况是 **`## main...origin/main [ahead 3]`**
（`26d3743` + `4ca0e06` + 本报告 §20 的文档提交）。另：工作树有一个**未跟踪的 `SMind/` 目录**，来源待核（不在本次验收范围）。

### 21.4 下一步（不占用用户时间）

1. **代码侧**：把 `scripts/verify/app-cdp.cjs` 扩成**三时点**（组词中 / 组词结束未提交 / 提交后）×
   （中文输入法 + 长 ASCII 无空格串），跑出 **行数 / 节点框宽 / 编辑区宽** 三组数字并贴出；
2. 读数达标 → 再请用户按 §20.2 走 A/B/C 六分钟清单；
3. 读数不达标 → **先怀疑量宽精度**（见 21.2-1），不要先怀疑草稿通道；
4. 官网下载**维持暂停**（`1ad0e98`），直到 21.4 全绿 → 恢复。
...
## 23. 🔴 公式被截断 / 溢出（2026-09-22 · CDP 实测 · **叶子节点**）

> **复现**（按用户要求**在叶子节点上**做，根节点的宽度上限与内边距不同、会掩盖问题）：
> 进编辑态 → `Tab` 建一个子节点（叶子）→ 在叶子里打一条长公式 → `Enter` 提交 → 量几何。

### 23.1 实测数据

```
叶子 · 长公式（提交后）:
  节点框 inline 宽 = 548px      ← 被上限钉死
  公式实际渲染宽   = 870px
  公式左超出      = +161px
  公式右超出      = +161px
```

- **行内公式**（标题里 `$…$`）：`.topic` 是 `overflow: visible` → 公式**溢出到节点框外**。
- **块级公式**（`topic.formula`）：`.topic__formula` 是 **`overflow: hidden`** → 公式被**裁掉**，即用户报的"截断"。

### 23.2 三处"写死的上限"（框永远撑不过它）

| 位置 | 上限 | 作用 |
|---|---|---|
| `src/renderer/src/render/formula.ts:86` | `Math.min(ceil(width) + 2, FORMULA_MAX_WIDTH * 2)` = **520px** | 能拿到 DOM 时的**实测路径** |
| `src/shared/layout/accessory.ts:73-76` | `FORMULA_MAX_WIDTH = **260px**` | 拿不到 DOM 时的**估算路径** |
| `src/renderer/src/styles/09-section.css:165-171` | `.topic__formula { overflow: hidden }` | **裁切框**：比它宽的部分直接看不见 |

`measure.ts:321-328` 把 `formulaBox.width` 计入 `contentWidth` ⇒ 节点框宽 = `min(公式宽, 520) + 内边距` ⇒
**超上限的公式必然放不下**。

**不是今晚的回归**：`FORMULA_MAX_WIDTH = 260` 出自 P4 提交 `cccb72e`，`v0.9.0` 同值。

### 23.3 参考修法（**仅建议，非要求**）

1. **让框跟着公式长**：把 520 这个硬上限提到只防呆的量级（或去掉），框按公式自然宽度撑开 → 绝大多数公式完整显示，与"公式块是原子块"的设计一致。
2. **等比缩小**（图片/代码块已有此机制）：超上限时把 KaTeX 字号按比例缩到刚好装进框内 → 不溢出，但很长的公式会变小。
3. **只解裁切**：`.topic__formula` 的 `overflow: hidden` 改 `visible` → 不再被切，但会溢出框外、可能压到别的节点。

### 23.4 可脚本化的验收判据（在**叶子**上做）

```
① 叶子节点里放一条长公式（自然宽 > 520px）→ 提交
② 量：公式元素 getBoundingClientRect().width ≤ 它所在的公式框/节点框宽度 + 1px
③ 断言：不出现"公式左/右超出 > 1px"（块级公式还不许被 overflow 裁掉）
④ 行内公式同样：节点框内宽 ≥ 行宽（不溢出）
五道门槛 + 断言净增照旧
```
...
## 25. §23 公式截断 —— QA 验收实测（2026-09-22）· **通过**

> **对象**：`a2a2b36`（实测路径上限 `FORMULA_MAX_WIDTH * 2`=520 → 新常量 `FORMULA_HARD_MAX_WIDTH`=4000 只防呆；估算路径同步）
> **测法**：与 §24 同一套（叶子节点 + `data-topic-id` 追踪 + Range 选取清空）；公式用 20 项 `\frac` 链（自然宽 > 520px）

### 25.1 改前 / 改后对照（同一测法）

| 读数 | 改前（`21bdc5b`） | 改后（`a2a2b36`） | 判定 |
|---|---|---|---|
| 节点框 inline 宽 | 548px（= 520 + 内边距，被上限钉死） | **1246px** | ✅ 框跟着公式长 |
| 公式实际渲染宽 | 870px | 870px | 不变 |
| 公式左/右超出 | **+161px / +161px** | **−188px / −188px**（负 = 在框内） | ✅ 不溢出、不被裁 |

⇒ **§23「公式被截断 / 溢出」记「已修（QA 实测）」**。

### 25.2 顺带两条观察（非本次缺陷，登记备查）

1. **框比公式实际渲染宽约 40%**：框 1246px，而节点内公式只有 870px（+ 内边距约 28px）。
   说明 `formulaSize()` 在**隐藏宿主**里量出的值与**节点内真实渲染**不一致（同一 `formulaSize` 被布局与渲染共用，
   CSS 注释里也写着"两者不该打架"）。**方向是"框偏宽"**——不裁切、不丢内容，属观感问题（P2），不是本次的截断。
2. **`FORMULA_MAX_WIDTH`（260）现在没有任何使用点**（`git grep` 只剩定义与注释）→ **死常量**。
   建议删除，或在注释里明确标"仅历史参考，别再拿来截断"。

### 25.3 仍未修（同一节点上同时测到）

编辑中（未提交）框仍为 **115px**，而文本需要约 870px —— 即 **D-07 的"编辑期测量不生效"照旧**（见 §24）。
公式这条修好**不改变** D-07 的状态。
...
## 27. D-07 复验受阻：**合成打字不可信**（2026-09-22，QA 自省 + 请用户 10 秒真键盘）

### 27.1 本轮做了什么

对 `ffae926`（把 override 的 `titleRich` 置空，正是 §26 建议的做法）重跑四组读数，并用三种合成输入各测一遍：

| 合成输入方式 | W1（打 20 字符后框宽） | 提交后（同文本） |
|---|---|---|
| `Input.dispatchKeyEvent { text }` | 76px（不变） | **228px** |
| `Input.insertText` | 76px（不变） | **228px** |
| 派发 ProseMirror 事务（`ed.pmViewDesc.view.dispatch`） | 脚本拿不到 view，**未跑成** | —— |

**⇒ 这两条读数是自相矛盾的**：若布局真的读不到文本，提交路径（读的就是同一份 store）也不该得出正确宽度。
⇒ **判据不足，本轮的 76→76 不能当作"仍没修好"的证据。**

### 27.2 修正我此前的两条结论（QA 自省）

1. **§24 的"`21bdc5b` 未修到点上"**：其读数用的是同一种合成打字 ⇒ **降级为「测法存疑，待真键盘复验」**；
2. **§26 的"根因 = `measure.ts:232` 只认 `titleRich`"**：**代码事实成立**（读码确认、`ffae926` 也照此改了），
   但**"改完仍 76px"这一实测结论不能算数**（合成输入污染），所以**§26 不能宣布"根因已验"，只能记"根因已定、修法已落、待真键盘复验"**。

### 27.3 下一步（两件事，一件给用户、一件给代码侧）

**① 用户 10 秒真键盘测试（唯一地面真值）**
> 双击任一节点 → 用**键盘**敲 20 个 `d` → 看那个蓝框**有没有跟着变宽**（再按 Enter 看是否跳回）。
> 这一步能一次判定 D-07 到底修没修好；合成事件永远替代不了它（IME/输入路径同理）。

**② 代码侧：把 `__layoutDiag` 补上"实际取值"（下一次读数就能一次定死）**
在 `use-canvas-layout` 的 `measure` 里记下**这一轮真正用到的**：
```
diag.title       = 实时用到的 title（字符串本身，或至少长度）
diag.draftLen    = editingDraftText.length
diag.editLen     = editingText.length
diag.richLen     = editingRich?.text?.length ?? -1
diag.editingWidth= 编辑节点这一轮的测量宽度（size.width）
```
有了这五个数，下一次不管用哪种输入方式，都能直接看出是"title 空 / 通道空 / 测量对但没写进节点"哪一格。
**顺带**：`scripts/verify/d07-accept.cjs` 的打字方式建议统一为 `Input.insertText`（或事务派发），
并保留你已有的"清空校验 + 测法不可信就退出 2"——那处做得对，本轮就是靠它才没出假结论。

---

## 28. D-07 真键盘复验（2026-09-22 用户截图）· **仍复现**

> **用户操作**：双击节点 → **真键盘**敲约 20 个字符 → **不按回车**。
> **看到的现象**：文字在**窄框里折成 7 行**（框只跟着变高，**宽度不变**）。

### 28.1 结论

1. **D-07 仍未修好** —— 真键盘（地面真值）复现，**`ffae926` 之后依然如此** ⇒ 不得记成已修。
2. **QA 的合成读数有效**：合成测得的「框 76px 不变、提交后 228px」与真键盘现象**逐条吻合**
   （窄框 + 只长高不长宽）。⇒ **§27 里对合成测法的降级可以撤回**：合成打字在"是否让框变宽"这件事上是可信的；
   之前那处"自相矛盾"的正确解释是 **提交路径与编辑期路径拿到的文本不同**，而不是测法失真。
3. 由此**§24 的结论恢复有效**：`21bdc5b`（缓存两处）与 `82fc870`（refresh 守卫）**都不是瓶颈**；
   `ffae926`（`titleRich: undefined`）**也没修好**。

### 28.2 现在的证据形态（把"该量但没量到"的那一格锁死在五个数上）

| 已知（实测） | 取值 |
|---|---|
| 编辑期走的分支 | `pass = incremental`（不是 `refresh`）✓ |
| override 是否命中 | `editingHits = 1`、`editingMisses = 0` ✓ |
| `useMemo` 是否重跑 | `memoRuns` 每键递增 ✓ |
| 编辑期框宽 | **76px 不变**（真键盘一致）✗ |
| 同一文本提交后 | **228px** ✓ |

⇒ 已排除：缓存分支、hot 未传、门未命中、memo 未重跑。
⇒ **唯一还没读到的就是"这一轮真正喂给测量的文本长度"** —— 这也正是下一轮必须补的五个数（见 §27.3-②）。

### 28.3 附带观察（低置信，待确认）

用户截图里每个 `d` 后面跟着一个 `'`（且带下划线）。**若是用户自己敲的，忽略**；
若不是，则可能与"选中后直接打字"的字符注入路径（`editor/typedChar.ts` + `takeTypedChar`）有关 ——
**建议单开一条**，别混进 D-07。

### 28.4 验收口径（最终）

**必须真键盘**：双击节点 → 键盘敲 20 个字符 → 不按回车 → **框宽必须跟着变宽**（现为 76px 不变）；
再按 Enter → 宽度不得跳变。合成输入只用于回归，不作为验收依据。

---

## 29. ✅ D-07 终局：真根因 = 归一化阶段的**节点对象复用**吞掉编辑期新测量（已修 + 已验）

> **执行者**：QA 侧（代码 agent 停机检修期间，按用户要求直接动手）。
> **判据链**：§28 的五个 `diag` 数 → `editingWidth = 228` 而渲染框仍是 `76` → 说明**测量对的，是布局把新结果丢了**。

### 29.1 根因（`src/shared/layout/core.ts` 平移+归一化那一步）
...
if (previous &&
  ...
  this.nodes[index] = previous            // ← 整块换回上一轮节点（含旧 width）
  continue
}
```

**编辑期工作簿没变 ⇒ `topic` 引用没变、坐标也没变 ⇒ 条件全中 ⇒ 把上一轮的节点对象整块换回来，
刚量出来的 228px 被静默丢掉**。注释里"复用必须同时看主题对象"的假设，在
**"测量源在 topic 之外"（编辑期：`editingText` / 实时草稿）**时不成立。

### 29.2 修法（最小且对症）

复用条件补两条：**尺寸必须相同**（`previous.width/height === node.width/height`）+
**热节点（`touched` 命中）一律不复用**。理由：复用携带 `lines` 等测量派生字段，
尺寸不同时复用一定是错的；编辑期整棵 hot 路径都不该复用。

### 29.3 真机等价验证（CDP，叶子节点，20 个字符）

| 读数 | 改前 | **改后** |
|---|---|---|
| 打字轨迹（每 5 字符） | 76 → 76 → 76 → 76 | **76 → 78 → 128 → 178 → 228** ✓ |
| W1 编辑期框宽 | 76px | **228px** ✓ |
| W3 提交后 | 228px | **228px**（不跳）✓ |
| `diag.editingWidth` | 228（早就对） | 228 ✓ |

### 29.4 顺手修的两件

1. **`use-canvas-layout.ts` 的中文注释又被写坏成 `?`**（第 104-106 行）—— **D-18① 复发**：
   仍是"脚本写文件没显式 UTF-8"。已恢复中文；**这条规矩要再强调一次**（改完必须 `git diff` 扫一眼中文）。
2. **`FORMULA_MAX_WIDTH` 是死常量**（§25.2 已记，未处理）。

### 29.5 门槛与断言

`typecheck / lint / format:check / selfcheck / verify` **全绿**；自检 **2879 → 2882（+3）**，
其中新增的是**真行为断言**（不是源码文本匹配）：
> 构造一棵小树，**同一 topic 对象**、只让测量宽度变（120 → 320 → 90），
> 断言节点宽度必须跟着变 —— 旧实现在 320 那一步就会失败（复用了 120 的旧对象）。

### 29.6 仍未收口（需要真键盘确认的一格）

自动化的「清空能缩回」读数为 FAIL（228 未缩回 76），但**很可能是测法所致**：
自动化清空用的是 `dispatchKeyEvent + Backspace`，而本轮已证明**这条路不走 ProseMirror**
（DOM 空了但 `editingText` 没变）。⇒ **请在真键盘上确认一次**：编辑态里全选删除，
框是否**跟着缩回**（这一步同时覆盖 D-14）。

---

## 30. D-14 根因（与 D-07 同族、方向相反）：**陈旧的草稿永久盖住真值**

> **触发**：用户真键盘复验 —— 编辑态里删掉全部文字，**框不回缩**（`29.6` 预判的那一格被证实）。
> **复现**：QA 的自动化同样复现（DOM 文本已空、框宽不变）⇒ **不是测法问题**。

### 30.1 诊断读数（清空后，同一节点）

```
DOM 文本        = ""     ✓ 已清空
diag.editLen    = 0      ✓ editingText 也清了（ProseMirror 事务确实发生）
diag.draftLen   = 20     ✗✗ 草稿通道仍留着旧文本！
diag.titleLen   = 20     ✗  于是 override 用的是草稿（旧文本）
diag.editingWidth = 208  ✗  测量自然还是 208 → 框不缩
```

### 30.2 根因

override 的取值是 **`editingDraftText || editingText`（草稿优先）**；
而草稿的上报当时挂在 **`requestAnimationFrame`** 上（"合并同一帧的多次组词事件"）。
**只要那一帧的 rAF 没跑到，草稿就永久停在旧值** —— 之后无论怎么删字，
`editingText` 再对也没用，因为它被草稿盖住了。

**性质**：这是与 D-07 相反方向的同一族缺陷 —— 都在"编辑期测量源的同步"上。
D-07 是"新值传不进去"，D-14 是"旧值传不出来"。

### 30.3 修法与验证（**已改源码 + 已实测通过**）

- 草稿上报改为 **同步 + 按值去重**（值没变就不写 store，所以同步不会带来多余重排）；
- 触发点从 4 个扩到 6 个：`compositionstart/update/end`、`input`、**`beforeinput`**、**`keyup`**。

**实测（同一叶子节点，构建后重跑）**

```
W0 空标题          76px
打字轨迹（每 5 字） 76 → 78 → 128 → 178 → 228   ✓ 跟着长
W1 编辑中 20 字     228px  ✓
W2 清空后           76px   ✓ 缩回去了（改前恒为 208/228）
diag：W1 titleLen=20 / draftLen=20 / editLen=20 / richLen=20 / editingWidth=228
      W2 titleLen=0  / draftLen=0  / editLen=0  / richLen=0  / editingWidth=76
```

**⇒ ①「打字让框变宽」②「清空能缩回」③「提交不跳」三组全 PASS。**
（另注：`richLen` 也从"恒 0"变成 20 —— 草稿同步之后编辑期整条链的取值都自洽了。）

### 30.4 教训（写进规矩）

**"为了性能引入的异步合并"（rAF / throttle / debounce）是"值永久停在旧态"类缺陷的高发地。**
凡是用它做同步的，必须配一条**最终一致的兜底**（同步上报、或在下一个事件里无条件对齐），
否则丢失的那一帧会变成永久状态 —— 本仓已有两例：本条的草稿通道、以及 §27 那次测法。
