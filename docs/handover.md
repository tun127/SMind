...
告诉我需要验证的项目」→「1,2测试没有什么问题 3,没问题 4,没问题 5,没问题」→「你不需要管推送我来做,你把本地代码层面管好就行」。

**这一段的提交**（每条都过了五道门槛）：

| 提交 | 内容 |
|---|---|
| `8093924` | **标签拖拽重排的源标签被自己覆盖**（可证缺陷：`tab === tabs[index]` + 进入分支的前提 `from !== index` ⇒ 那行赋值必然换成被悬停的标签）→ 删掉那行；顺带订正 `use-keyboard-shortcuts.ts` 的注释漂移 |
| `420389c` | 上述同步进 `known-issues` / `handover` / `P1-acceptance` |
| `ddf08ab` | **收掉 `refactor-audit.md` §七 的「唯一遗留」**：实体解码的扫描与分派收进 `shared/entities.ts`（`decodeEntityReferences` / `decodeEntityBody`），三处**名字表与查表口径按调用点保留**；自检 +12 条 |
| `40f7b8d` | 把「唯一遗留」标记为已收口（四处文档同步） |
| `fb4adbd` | **主进程回归网**：`main/doc-resources.ts` 与 `main/document.ts` 纳入自检（+22 条，2672 → **2694**） |
...
- 另一会话那笔**删代码**的提交 `2ffaac1` 单独验过：3 个符号（`CodeLanguage` / `CODE_CHAR_WIDTH` /
  `richIsEmpty`）在 `src` + `scripts` 全仓**零引用**（只剩文档里的历史记录），删除安全；
- `refactor-audit.md` §七 的复核结论逐条读过，其中「XML 实体解码 3 份」经实测比原结论更细
  （数值解码早已统一，重复的只是扫描/分派）→ 已按 E1 判据收口。

**用户验收结果（2026-09-19 深夜，本机开发版，原话「没有出现任何问题」「1,2测试没有什么问题 3,没问题 4,没问题 5,没问题」）**：
关窗链路 / 一整轮真实 AI 对话 / 画布拖拽全形态 / **撤销后草稿跟着回退** / 搜索筛选染色·折叠锚点·视角锁定
**全部通过**。明细已入册 `handover.md` §五 与 `P1-acceptance.md`；`ops.md` #20 的状态已改为
「高危五项已由用户实测通过」，**打包剩下的阻塞只有 PDF 矢量验收（#6）**。

**仍未逐条走过的界面细项**（风险已大幅下降，按需再验）：A6-3 回合收尾细分项、A4–A7 更早各批
（面板三条分支与四段、工具栏各组与「收进更多 ▾」、外壳菜单/快捷键/拖文件、自动保存与崩溃恢复提示、
窗口标题与多窗口聚焦、A3 子组件与更早 4 项）。

**主进程测试网的边界（已拍板，别再提"补测"）**：剩下六项（`menu.ts` 结构、`lifecycle.ts` 的锁与装配顺序、
`protocol.handle` 注册、`ipc/*` 入参校验细节、`license` 状态落盘与 Pro 缓存、`update` 与 electron-updater 的交互）
**刻意不建网**——判据是「静默失败 ＋ 会伤数据或收入 ＋ 能纯函数化」三条同时满足才值得，
而它们要么失败看得见、要么危险的那半已抽出来测完、要么根本不是静默的；且为它们引入 Electron mock
会破坏本仓库「零 mock（唯一例外是 `fakeMeasure` 这个纯函数替身）」的架构选择。
完整论证与逐项理由见 `known-issues.md` 的「主进程回归网 + 两类契约守卫」一节。
...
> ### ✅ 已由用户实测通过（2026-09-19 深夜，本机开发版；原话「目前开发版没有出现任何问题」）
>
> | 已验项 | 结果 |
> |---|---|
> | **关窗链路**：多标签逐个询问 / 中途取消不丢标签 / 保存失败不关窗 / 启动即关窗不清自动存档 / 崩溃恢复弹窗 | ✅ 无问题 |
> | **一整轮真实 AI 对话**：工具调用 · 破坏性确认 · 中途停止 · 切文档中止 · 整轮改动一步撤销 | ✅ 无问题 |
> | **画布拖拽**：吸附成子主题（含提示文案）/ 同级插入线 / `Alt` 自由摆放 / 多选整群拖 / 左右对调预览 / 贴边自动滚动 | ✅ 无问题 |
> | **撤销后草稿跟着回退**（2026-09-19 修的那条；判定路径：改 → 点别处 → `Ctrl+Z` → 再点进点走，内容不该回来） | ✅ 无问题 |
> | **搜索筛选染色 · 折叠锚点（被折叠节点原地不动）· 视角锁定** | ✅ 无问题 |
>
> **仍未逐条走过的**（按需再验，风险已大幅下降）：A6-3 回合收尾细分项、A4–A7 更早各批
> （面板三条分支与四段、工具栏各组按下/禁用提示与「收进更多 ▾」、外壳菜单命令逐条、快捷键、
> 拖文件三种去处、自动保存与崩溃恢复提示、窗口标题与多窗口聚焦、A3 的 9 个子组件与更早 4 项）。
