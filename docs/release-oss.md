...
4. **梯子开成 TUN/全局模式时，本机的 curl 读数会被污染**（2026-09-22 实测）：同一批次里 `latest.yml` 正常、而 exe 的 ranged 请求却返回 404（正文 356 字节的 NoSuchKey XML），改走代理又变成 200 —— `--noproxy '*'` 也挡不住（TUN 在网络层拦截）。**结论：本机读数抖动不等于线上有问题**；判定线上以①上传脚本自带的直链校验（含字节数）②浏览器实际打开/客户端真实更新为准。
