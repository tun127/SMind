...
3. 🔴 **许可码不唯一（最大障碍）** —— `LicensePayload` 没有序列号字段，签名覆盖的就是 payload 字节，因此**同一 payload 签出来的码逐字相同**。批量预生成 200 个早鸟码会得到 **200 个一模一样的码**：限量 200 形同虚设、泄露无法定位、对账无从谈起。**卡密池方案没有这一项就不能开工。**
...
3. serial 连续可读（`EB-000001`），批次前缀区分早鸟 / 正价；
...
支撑两件事：① KPI 表「许可签发数 ≈ 付费单数」的对账；② **早鸟 200 份的余量统计**（限量必须能数得清）。
