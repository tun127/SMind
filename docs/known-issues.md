...
| **本轮审计剩下的几类（未做完，仍在清单上）** | **重复实现收敛**还剩 5 组：outline 方言两处解析（`ai/outline.ts` 与 `import/markdown.ts` 只靠约定一致）、语言清单三处（`lang-defs` / `code-language` / `tools-write`）、`lexer.ts` 的引号扫描三份、`measure.ts` 与 `export/raster.ts` 的 `font` 串、`ancestorsOf`+`findTopic` 的祖先链三份；**死导出**（审计给的是 A 类 72 处 + B 类 129 处，但同一份审计的三条结论已被证伪，这份清单必须逐条自行 grep 复核后再删，且 grep 要同时覆盖 `src/**` 与 `scripts/**`）；**快照作用域/配额**（恢复不按文档隔离、单文档版本数无上限）；**边界必须落在连续同级之间**这条约束目前依赖调用方自觉；`viewportActions` 的收尾清理 |
