...
### 2026-09-22 新增登记：无法创建「独立主题」（浮动主题）——**用户已拍板进 0.9.3**

**现象**：应用能**读**独立主题（导入含 `children.detached` 的 `.xmind`）、大纲/搜索/统计/AI 都已覆盖它（D-04 修复后），
但**没有任何入口能创建一个**，也没有"把独立主题放回结构里"的入口。

**代码事实（2026-09-22 全仓核实）**：

- 全仓**没有任何一处**把主题写进 `detachedChildren`；`detachTopic()` 是"从父级摘下来"，只被删除（`structure.ts`）与移动（`move.ts`）调用；
- `attachChild()` 只会写 `children`，**不会**写 `detachedChildren`；
- 已支持的一侧（**不用改**）：模型字段（`types.ts`）、`walk` / `allChildrenOf` / `countTopics` / `cloneTopicDeep`、
  大纲两种模式都列（`outline/index.ts`）、搜索（`search/index.ts`）、资源收集（`resources.ts`）、
  变更检测（`incremental.ts`）、导入导出 **round-trip 完整**（`parse.ts` 读 / `serialize.ts` 写 `detached` / 旧版 xmind8 也读 `detached` group）；
- ⚠️ **只有 `root.detachedChildren` 参与布局与分支配色**（`layout/core.ts` 的 `markFloating` 只从 `root.detachedChildren` 出发，
  且它**只管配色索引、不定坐标**）→ **非根级的 `detachedChildren` 在画布上没有布局语义**（导入文件里若出现在深层，会静默不显示）；
- ⚠️ **未查清、必须先定死**：浮动主题在画布上的 x/y 到底从哪来（`position` 是绝对坐标还是相对偏移？没有 `position` 时会落在哪）。

**待补的两件事**：① 创建（把选中主题移入 `root.detachedChildren`，并给它一个落在原位的坐标）；② 吸回（移回某个父级的 `children`，并清掉 `position`，可复用 `moveTopic` + 清偏移）。

**与「自由摆放」的区别（别混）**：自由摆放 = 主题**仍在树里**、只多了 `position` 偏移（拖到空白处 / 按住 Alt 拖即可，工具条「恢复自动布局」可放回）；
独立主题 = **脱离树**，不参与自动布局。

### 🔴 2026-09-22 实测追加：独立主题**在画布上根本不会被渲染**（真缺陷，创建功能的硬前置）

**用户实测**：导入一份含独立主题的 `.xmind` 后，AI 能搜到它、也把画布选择设到了它，**但用户在画布上看不见它**（截图取证）。

**探针实测**（隔离实例直接打开测试文件，`--remote-debugging-port=9222`，读全部 `.topic` 的包围盒）：

```
画布上的 .topic 元素数: 10
其中匹配「第四季度」的: []      ← 一个都没有（其余 10 个都是树上节点，全部可见）
```

→ 结论：**不是位置偏、也不是被遮挡，而是没有 DOM 元素** —— 布局层不给 `root.detachedChildren`
产出任何 x/y（`layout/**` 里对 detached 只有两处：`core.ts` 的**分支配色索引**与 `incremental.ts` 的**变更收集**，
**没有产出图元**），于是图元层什么都不画。**导入的浮动主题同样看不见**（与"格式保真"卖点直接冲突）。

**因此"创建独立主题"这个功能必须连带做渲染支持**，否则造出来也是一片空白。建议范围（一条线做完）：

1. **布局支持**：为 `root.detachedChildren` 产出图元（坐标来源需先定口径：`position` 是绝对坐标还是相对偏移、缺省时放在哪）；
2. **创建入口**（右键「变为独立主题」/ 大纲 `Shift+Tab`）：位置**原地不动**（偏差 <1px）；
3. **吸回**（右键「放回结构」）：复用 `moveTopic` + **清 `position`**；
4. **视觉区分**（虚线边框/浅底），并让文案与「自由摆放」区分开；
5. 回归：D-04 那批（AI 寻址/搜索/大纲/统计）必须仍绿。

**影响面与优先级**：属既有行为（非本轮引入），影响的是"从 Xmind 导入含浮动主题的文件"这一类用户
（人数不多，但正是"格式保真"的目标人群）。**不阻塞 0.9.3**，建议与创建功能同一批做（两者互为前置）。
