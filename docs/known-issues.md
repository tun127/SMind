...
| **本轮审计剩下的几类（未做完，仍在清单上）** | **重复实现收敛**还剩 4 组：outline 方言两处解析（`ai/outline.ts` 与 `import/markdown.ts` 只靠约定一致）、语言清单三处（`lang-defs` / `code-language` / `tools-write`）、`lexer.ts` 的引号扫描三份、`ancestorsOf`+`findTopic` 的祖先链三份（`font` 串已收敛为 `cssFontOf`，测量与导出共用）；**死导出**（审计给的是 A 类 72 处 + B 类 129 处，但同一份审计的三条结论已被证伪，这份清单必须逐条自行 grep 复核后再删，且 grep 要同时覆盖 `src/**` 与 `scripts/**`）；**快照配额**（单文档版本数上限，见 `shared/snapshot` 的 `prune`）——**作用域已修**：恢复前按 `snapshotOwnerKey` 校验归属，不一致直接拒绝，避免把别的文档内容写进当前文件；**边界必须落在连续同级之间**这条约束目前依赖调用方自觉；`viewportActions` 的收尾清理 |
