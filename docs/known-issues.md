...
| **`noUncheckedIndexedAccess`** | **已量出真实代价**：开启后共 **504 处**报错——其中 **304 处集中在 `scripts/selfcheck.ts`** 的测试断言里，源文件约 **200 处**，最密的是 `shared/code/highlight.ts`(47)、`shared/import/markdown.ts`(29)、布局若干文件与 `main/index.ts`。这是一次**五百处量级**的改动，且正确做法是逐个判断"这里真的可能缺吗"而不是机械加 `!`（机械加 `!` 比不开还糟：把编译器的提醒换成了谎话）。必须**单独排一轮、单独一个提交**，不能混进功能改动里 |
