...
## 待处理（2026-09-21 第二轮复验 D-04～D-16）

> 说明：上一节登记的是**第一轮 F1–F5**；第二轮的 D-04～D-16 原先只存在于
> `docs/defect-report-2026-09-21.md`（含 §7 复验结论）。按该报告 §7.1 的提醒，
> 这里补一条索引，免得按本文件排期时整批漏掉：**D-04～D-16 以报告为准**。

### 已修（本轮，代码侧）

| 条 | 内容 | 提交 |
|---|---|---|
| D-01 | 撤销栈满 200 后 AI 回合「一步撤销」静默失效 | `cbc1ead` |
| D-09~D-12 | 移动失败却改树 / 负数 index 夹成 0 / 缓存计时恒 0 / 拖拽监听器泄漏 | `517fc6b` |
| D-03 | 写盘期间的新编辑被误标「已保存」（新增 `docRevision`） | `49fccfe` |
| D-16 | 上传脚本缺 `rt.yml` → rt 通道收不到更新、首跑验收会假通过 | `03343ab` |
| D-15 | 启动 ≈1 秒抢跑一次检查（= T3；§8 红线的受控例外） | `f66b073` |

### 未修（下一轮）

- **D-02** 多标签下崩溃恢复只覆盖激活标签（P0）。⚠️ 修法要动 **IPC 契约**：
  `clearAutosave()` 必须能带 `docId`（否则任一标签保存就会清掉别的标签的存档），
  而红线明令"不动 IPC 契约"→ **需要一次受控例外批准**再动。
- **D-07 / D-08** 组词期加宽被 flex-shrink 抵消 + 两个 effect 抢写 `dom.style.width`（P1，用户已复现）。
  ⚠️ 同时更正：`515aac2` 里的 A3 **实测无效**——放开的是 `max-width`，真正卡住的是 flex `shrink`。
  这条不能再按"已修"记；对外说明与 CHANGELOG 不得出现 A3 已完成。
- **D-04** `shared/agent/**` 对浮动主题整块失明（7 处遍历 + 建议加静态口径断言）。
- **D-05** 上下文压缩窗口漏掉 `recent` 里除最后一条外的 `toolNotes`。
- **D-06** 同一回合按参数文本判重 → 文档已变时重复读调用拿到旧结果（可复用 `docRevision`）。
- **D-13** 第 1 步「内容变少就确认」（`setNotes` / `setCode` / `setFormula`）。
- **D-14** 清空标题后节点不缩回：**必须先跑报告 §3 的探针**定死甲/乙/丙，再动手。

### 口径收紧：overlay 富文本只是「部分完成」

`6c3f078` 交付的是**数据层 + 画布渲染 + 手写标记输入**，不是完整功能。准确口径是：

- ✅ **已支持**：`部分加粗 / 高亮 / 等宽 / 上下标 / 删除线`——靠**手写标记**表达
  （`**粗体**`、`==高亮==`、`` `等宽` ``、`^上标^`、`~下标~`），画布按 run 分段渲染，`.xmind` 往返保真；
- ❌ **未支持**：**选中几个字 → 点按钮**（图形化编辑器还没做，`nodePanel/overlay-branch.tsx` 的
