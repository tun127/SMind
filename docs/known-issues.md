...
| **本轮审计剩下的几类（未做完，仍在清单上）** | **重复实现收敛**还剩 3 组：outline 方言两处解析（`ai/outline.ts` 与 `import/markdown.ts` 只靠约定一致）、语言清单三处（`lang-defs` / `code-language` / `tools-write`）、`ancestorsOf`+`findTopic` 的祖先链三份（`font` 串已收敛为 `cssFontOf`；`lexer` 的引号扫描已收敛为 `scanQuoted`；祖先链已收敛为 `ancestorTitlesOf`/`titlePathOf`）；**死导出**（审计给的是 A 类 72 处 + B 类 129 处，但同一份审计的多条结论已被证伪，这份清单必须逐条自行 grep 复核后再删，且 grep 要同时覆盖 `src/**` 与 `scripts/**`）；**边界必须落在连续同级之间**这条约束目前依赖调用方自觉；`viewportActions` 的收尾清理。<br>**快照一项已全部落地**：作用域＝恢复前按 `snapshotOwnerKey` 校验归属；**配额本来就有**（`shared/snapshot` 的 `prune`：`SNAPSHOT_LIMITS.perDoc` 上限、手动版本占 `manual` 名额优先保留、超出按时间裁掉并删文件），审计说"无上限"**不成立** |
