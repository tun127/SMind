...
| **`noUncheckedIndexedAccess`** | **部分完成（已启用并常驻检查）**：全仓共 504 处（304 处在自检脚本的断言里）。已修 **72 处**，并把**引用闭包已干净**的部分纳入常驻检查：`theme` / `ai` / `model` / `richtext` / `xmind` / `render` / `main`（共 **49 个文件**，配置在 `tsconfig.strict.json`，由 `npm run typecheck` 一并运行）。剩下 **128 处**在 `shared/code`(47)、`shared/layout`(34)、`shared/import`(30)、界面组件等处。**推进方式**：清理完一个目录后把它加进该配置的 `include`；但**务必先确认它的引用闭包也干净**——`include` 只是入口，TypeScript 会顺着 import 纳入被引用文件，否则 `typecheck` 会立刻变红、CI 跟着失败。修法一律按语义处理（提前退出 / 兜底值 / 只有确证不可能缺时才断言），不做机械 `!` |
| **react-hooks v6 的 `refs` / `immutability` / `purity`** | 属于 React Compiler 的写法建议，会指着画布里**有意为之**的 `xxxRef.current = yyy` 报警。照改等于给画布做一次重构，另立一项。目前这三条关闭 |
