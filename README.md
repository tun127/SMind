...
| 代码块 | 节点属性面板「代码块」：选语言 + 贴代码，节点内**按语言语法高亮**（js/ts/python/java/c/cpp/csharp/go/rust/sql/json/yaml/bash/html/css，注释/字符串/关键字/函数名/类型/键各有配色），等宽排版、超出行数可滚动；导出图片 / PDF 同样带色 |
...
| 多窗口多文件 | 菜单「文件 → **新建窗口**」（`Ctrl+Shift+N`）或工具栏「更多▾」：一个窗口一份文档，各自独立的资源、自动存档与撤销栈；**「在新窗口打开当前画布」**可把同一份文件的某张画布开在第二个窗口里，用来并排看两张画布；双击 / 命令行 / 拖入的 `.xmind` 若已开着就**聚焦那个窗口**，否则**开新窗口**；退出时每个窗口分别确认未保存内容 |
