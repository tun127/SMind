...
> 一个**本地**思维导图软件（对标 Xmind）。原生文件格式就是 `.xmind`，可直接与 Xmind 互通。
...
| 方式 | 怎么做 | 适合谁 |
|---|---|---|
| **免安装版**（推荐） | 双击 `Mind-<版本>-x64-portable.exe` 即用 | 自己用、发给别人 |
| **安装版** | 双击 `Mind-<版本>-x64-setup.exe`，可选安装目录，自动建桌面/开始菜单快捷方式 | 长期固定在一台机器上用 |
...
**你的数据存在哪**（都在 `%APPDATA%\Mind`，不在 exe 旁边）：
...
- **Key 只存在本机**（`%APPDATA%\Mind\ai-config.json`），界面上只显示掩码；不写进 `.xmind`
...
**Q：双击打包版没反应？**
同一时间只能开一个 Mind。若已经开着一个（包括 `npm run dev` 的开发版），第二个会直接退出。

**Q：双击 `.xmind` 打不开 / 想用 Mind 打开本地文件？**
- **安装版**安装时会注册 `.xmind` 关联：装完后在资源管理器里**双击即可**，或右键 →「打开方式 → Mind」（想一直用它打开就选「始终」）。
- **免安装版**不改系统（绿色软件）：把文件**拖到 `Mind-…-portable.exe` 上**，或**拖进已打开的 Mind 窗口**，即可打开。
- Mind 已经开着时再双击一个文件：会直接开在**当前窗口**里，不会另起第二个进程。
- 应用里随时 `Ctrl+O`。
...
npm run selfcheck    # 自检断言（当前 1128 项）
